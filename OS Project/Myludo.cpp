...
void menu(int &choice, int &players)
{
    ...
    if (choice == 1)
    {
        ...
        {
            cout << endl
                 << "How many Tokens (between 2 and 4) ?" << endl
                 << endl
                 << ">";
            // numTokens = 2;
            cin >> numTokens;
    ...
    if (currtok != NULL)
    {
        ...
        if (p.complete == numTokens)
        {
            ...
            if (!already)
            {
                ...
                nbPlayer--;
                cout << "Erased " << p.color << p.team + 1 << RESET << endl;
                if (nbPlayer == 1)
                {
                    for (int i = Winners.size(); i >= 0; i--)
                        if (Winners[i] == NULL)
...
void *playerT(void *arg)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        token &curr = p.goti[choice - 1]; // first index is whose turn it is and the 2nd index is which goti
